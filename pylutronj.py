...
class LutronConnection(threading.Thread):
    ...
    connection defaults like turning off the prompt, etc."""
        ...
        _LOGGER.info("Connected to Lutron host %s" % self._host)
    ...
    def _maybe_reconnect(self):
        ...
        with self._lock:
            if not self._connected:
                _LOGGER.info("Reconnecting to Lutron host %s" % self._host)
...
class LutronXmlDbParser(object):
  ...
  etc. are not implemented."""
    ...
    relevant Lutron objects and stuffs them into the appropriate hierarchy."""
        ...
        _LOGGER.debug("Beginning XML parser")
        ...
        # Areas later.
        _LOGGER.debug("Beginning OccupancyGroups parser")
        ...
        _LOGGER.debug("Beginning Areas parser")
        top_area = root.find('Areas').find('Area')
        self.project_name = top_area.get('Name')
        areas = top_area.find('Areas')
        _LOGGER.debug("Top Area name %s, found %d child Areas" % (self.project_name, len(areas)))
    ...
    lights/outlets, etc."""
        ...
        _LOGGER.debug("Beginning Output parser for %s, a %s" % (kwargs['name'], output_type))
    ...
    def _parse_button(self, keypad, component_xml):
        ...
        if not name:
            name = "Unknown Button"
        _LOGGER.debug("Beginning Button parser for %s, engraved %s" % (name, engraving))
    ...
    def _parse_cci(self, keypad, component_xml):
        ...
        name = f"{cci_type} {component_number}"
        _LOGGER.debug("Beginning CCI parser for %s" % name)
    ...
    """
        ...
        _LOGGER.debug("Beginning MotionSensor parser for %s" % name)
